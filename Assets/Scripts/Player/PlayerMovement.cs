...
public class PlayerMovement : MonoBehaviour {
    ...
    IEnumerator DelayedVelocityReset() {
    ...
    public IEnumerator damageWhileSlahsing(Transform transform, float range) {
        ...
        while (attackDamageTimeLeft > 0) {
            ...
            for (int i = 0; i < hits.Length; i++) {
                ...
                if (hits[i].collider.gameObject.CompareTag("Destructible") &&
                    !destructibles.ContainsKey(hits[i].collider.gameObject.GetHashCode())) {
                    destructibles.Add(hits[i].collider.gameObject.GetHashCode(),
                        hits[i].collider.GetComponent<DestructibleBox>());
                }
                else {
